...
>>>>>>> 30f912cc414bbb6af1241903b44e85ad40dc7445:Energetic_Profile-Based_Protein_Comparison.Rmd
...
```{r}
df <- read.delim('COFACTOR_Test_TrueLabels.txt')
df$class <- substr(df$EC.Number,1,1)
Nprotein <- nrow(df)
#----------------------------------------------
E210_Enzyme <- data.frame(pdbID = df$PDB.id,
                            length = rep(NA,Nprotein),
                            class = df$class,
                            matrix(NA,ncol=210, nrow=Nprotein))

colnames(E210_Enzyme) = c("pdbID","length","class",pair_inter)
E210.seq_Enzyme <- E210_Enzyme_SS <- E210_Enzyme

for (NP in 1:Nprotein) {
  print(NP)
  ch <- substr(E210_Enzyme$pdbID[NP],5,5)
  pdbname <- substr(E210_Enzyme$pdbID[NP],1,4)
  E210_Enzyme$pdbID[NP] <- E210_Enzyme$pdbID[NP]
  E210_Enzyme_SS$pdbID[NP] <- E210_Enzyme$pdbID[NP]
  E210.seq_Enzyme$pdbID[NP] <- E210_Enzyme$pdbID[NP]
  tryCatch({
    pdb0 <- read.pdb(tolower(pdbname))
    sele1 <- atom.select(pdb0,"protein",type = "ATOM", chain=ch)
    sele2 <- atom.select(pdb0, "protein", elety=c("H","OXT"),inverse=TRUE)
    ...
    pdbXca <- pdbX[pdbX$elety == "CA",]
    seq <- aa321(pdbXca$resid)
    E210.seq_Enzyme$length[NP] <- length(unique(pdbX$resno))
    freq <- c()
    for (j in 1:20){
        freq[j] <-  length(grep(tolower(letters_list[j]),tolower(seq)))
    }
    ...
    E210_Enzyme$length[NP] <- length(unique(pdbX$resno))
    E210_Enzyme_SS$length[NP] <- length(unique(pdbX$resno))
    # -------------------
    new_pdb <- trim.pdb(pdb0, sele,verbose = FALSE)
    Net_Frame <- NetworkFrame(PDB = new_pdb)
    energy210 <- Energy_PC210(Net_Frame,energy_dell_dunbrack)
    E210_Enzyme[NP,4:213] <- energy210
    # ****************keep res helix & sheet************************
    indhs <- inedx_HelixSheet(PDB = new_pdb)
    Net_Frame_hs <- Net_Frame[Net_Frame$resnoif %in% indhs & Net_Frame$resnojf %in% indhs,]
    if(dim(Net_Frame_hs)[1]!=0){
    ...
    energy210_ss <- Energy_PC210(Net_Frame,energy_dell_dunbrack)
    E210_Enzyme_SS[NP,4:213] <- energy210_ss
    }, error = function(e) {
     cat("Error downloading file:", e$message, "\n")
    })
}
