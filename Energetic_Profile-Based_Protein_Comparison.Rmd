...
spike <- spike[spike$Conformation == "Close",]
spike <- spike[-grep('6XR8',spike$pdbID),] # There is no PDB file of ID 6XR8
Nprotein <- nrow(spike)
#----------------------------------------------
num_columns <- 210
E210_spike_close <- data.frame(pdbID = spike$pdbID,
                            length = rep(NA,Nprotein),
                            virus = spike$VIRUS,
...
for (NP in 1:Nprotein) {
  ...
  E210_spike_close$pdbID[NP] <- spike$pdbID[NP]
  tryCatch({
    ...
    pdbXca <- pdbX[pdbX$elety == "CA",]
    seq <- aa321(pdbXca$resid)
    E210_spike_close$seq[NP] <- paste0(seq,collapse = '')
    E210_spike_close$length[NP] <- length(unique(pdbX$resno))
    # -------------------
...
tm <- read.csv('Data/csv/TM_vec_bac.csv',row.names = 'X') 
# ---------------------------------------------------------
# ------- ENERGY
# ---------------------------------------
