...
ggarrange(tm_p,cpe_p,spe_p,ncol = 3)
...
gspe = ggplot(ump_df, aes(UMAP1, UMAP2, color= globin)) +
  ...
  ggtitle("SPE") +
  mytheme() 
ggarrange(gtm,gcpe,gspe,ncol = 3)
...
#----------Neighbor joining---------
spikeTree_nj <- nj(d)
p_data <- ggtree(spikeTree_nj, layout = 'dendrogram',
                 size = 1,branch.length = 'none')
t_nj<-groupOTU(p_data, grp, "groups") + aes(color=groups) +
...
mean_dis <- matrix(NA, nrow = 3, ncol = 3,
...
# ------------------------------------------------------------------------------
dists <- list(CPE=dis_CPE,SPE=dis_SPE,seqid=dis_seqid,TM_vec=tm,RMSD=rms)
...
colnames(mean_pair_dist) = c("MERS-CoV vs SARS-CoV", "MERS-CoV vs SARS-CoV-2", "SARS-CoV vs SARS-CoV-2")
library(reshape2)
...
# Plot the bar plot
ggplot(group_dis, aes(x = Comparison, y = Distance, fill = Method)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  labs(title = "Comparison of Different Distance Metrics Between Virus Pairs", 
       x = "Virus Comparison", y = "Distance") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 12),
        plot.title = element_text(hjust = 0.5, size = 16))


 ggplot(group_dis, aes(x = "", y = Distance, fill = Comparison)) +
     geom_bar(stat = "identity", width = 1) +
     coord_polar("y") +
     facet_wrap(~Method) +
     labs(title = "Pie Chart of Distance by Metric",
          x = NULL, y = NULL) +
     theme_void() +
...
df_tot_40 = df_tot[df_tot$data %in% "Astral 40",]
df_tot_40$dist = (df_tot_40$seq/2000 - df_tot_40$str/1000)
...
result <- mapply(function(x, y) manhat(x, y),
                 split(df_seq[,-c(1:8)], 1:nrow(df_seq)), 
...
cor(df_tot_40$length,df_tot_40$dist, use = "complete.obs")
plot(df_tot_40$length,df_tot_40$dist)
df_tot_40_60 = df_tot_40[df_tot_40$length < 60 , ]
cor(df_tot_40_60$seq,df_tot_40_60$str)
df_tot_40_100_200 = df_tot_40[df_tot_40$length < 200 & df_tot_40$length > 100 ,]
cor(df_tot_40_100_200$seq,df_tot_40_100_200$str)
df_tot_40_200_300 = df_tot_40[df_tot_40$length < 300 & df_tot_40$length > 200 ,]
cor(df_tot_40_200_300$seq,df_tot_40_200_300$str)
df_tot_40_300_500 = df_tot_40[df_tot_40$length < 400 & df_tot_40$length > 300 ,]
cor(df_tot_40_300_500$seq,df_tot_40_300_500$str)
df_tot_40_500 = df_tot_40[ df_tot_40$length > 400 ,]
cor(df_tot_40_500$seq,df_tot_40_500$str)
df_tot_40$dist = df_tot_40$seq - df_tot_40$str
plot(df_tot_40$length,df_tot_40$dist)
# ----------------------------------------------------
...
calculate_entropy <- function(protein_sequence) {
  ...
  entropy_value <- entropy(aa_counts)
  return(entropy_value)
}
astral40$complexity <- sapply(astral40$seq, calculate_entropy)
...
for (NP in 1:nrow(E210_CT_Ho)) {
  ...
  tryCatch({
    pdb0 <- read.pdb(pdbname,verbose = FALSE)
    sele1 <- atom.select(pdb0,"protein",type = "ATOM", chain=substr(E210_CT_Ho$pdbID[NP],5,5))
