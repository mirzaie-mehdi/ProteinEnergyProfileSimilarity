...
for (i in 1:6) {
  ...
  p <- ggplot(df2, aes(length, Value))+
    ...
    ylab('Distance across CPE and SPE') +
...
p<-ggplot(df, aes(n_protein, time/sumAA, col=method)) +
  ...
  stat_poly_eq(aes(label = paste(..eq.label..)),
               data = subset(df, method == "TM-Vec"),  
               formula = y ~ x,label.y = 0.98, 
               parse = TRUE,size = 7, color = "magenta") +  
  stat_poly_eq(aes(label = paste(..eq.label..)),
               data = subset(df, method == "CPE"),   
               formula = y ~ x, label.y = 0.9,
               parse = TRUE,size = 7, color = "orange") + 
  xlab('N Protein')+
