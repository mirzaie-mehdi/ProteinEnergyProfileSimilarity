...
Pij_rm_Olaps <- read.csv("Data/csv/Pij_rm_Olaps.csv", header = F,sep = ";")
Pij_rm_Olaps <- (Pij_rm_Olaps + t(Pij_rm_Olaps))/2

cor(Pij[upper.tri(Pij,diag = T)],
    Pij_rm_Olaps[upper.tri(Pij_rm_Olaps,diag = T)])
...
```{r}
source('Functions.R')
astral <- readRDS("Data/rds_old/E210.seq_astral_95.rds")
df <- astral[!is.na(astral$FF),]
dis <- as.matrix(proxy::dist(df[,-c(1:8)], method = manhat))
diag(dis) <- NA

tm <- read.csv('Data/csv/disTM_vec_astral95.csv')
tm <- tm[!is.na(astral$FF), !is.na(astral$FF)]
tm <- 1 - tm
diag(tm) <- NA
...
dis2 <- dis[astral$fold%in%names(sf), astral$fold%in%names(sf)]
...
res$TP <- ifelse(res$source==res$target,1,0)
# Accuracy
round(sum(res$TP)/nrow(res),2)
...
dis_SPE <- as.matrix(proxy::dist(df[,-c(1:2)], method = manhat))
t_SPE <- tictoc::toc()
...
dis <- list(dis_CPE, dis_SPE, tm)
summ <- lapply(dis, summary_1NN, df$sars_proteom)



rbind(c('TM',  summ[['TM']][1,],  t_tm),
      c('CPE', summ[['CPE']][1,], t_CPE),
      c('SPE', summ[['SPE']][1,], t_SPE))
...
tree_data <- ape::bionj(as.dist(dis))
p_data <- ggtree(tree_data, layout = 'unrooted',
                 branch.length='none',size = 1.5) 

# myBoots <- boot.phylo(tree_data, tm, B = 100,
#                       function(e) bionj(as.dist(dis)))
# 
...
t_CPE<-groupOTU(p_data, grp, "groups") + aes(color=groups) +
  ...
  scale_color_discrete(breaks=c('Bacteri', 'BMM_alpha', 'BMM_beta',
                                'Dps','Fatty_acid','Ferritins','RNRR2',
                                'Pfam_1otkA','Rubrerythrins'),
                       type =  c('red',"orange2","yellow4","green3",
                                 'skyblue3','blue','black','magenta','purple2'))

nwik_SPE <- write.tree(tree_data)
nwik_SPE <- gsub(":[^,)]*", "", nwik_SPE)
