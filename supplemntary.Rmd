...
tm2cv <- aggregate(score~status+method,tm2,cv)
tm2cv$method <- factor(tm2cv$method,levels = c('TM_vec','Alphafold2','Omegafold','ESMfold','CPE'))

p <- ggplot(tm2cv,aes(status,score,fill=method))+
...
```{r}
mat <- read_excel("~/Desktop/proj/C0023434_disease_gda_summary.xlsx")
mat <- mat[mat$N_PMIDs>1,]
tdis <- unique(mat$UniProt)

df0 <-readRDS('../../DTI/241/E210.seq_dtcMathew.rds')
df0 <- df0[df0$target%in%mat$UniProt,]
dt <- unique(df0$target)

m <- match(tdis, dt)
tdis <- 

sab <- readRDS('../../DTI/241/sAB.rds')
sab <- sab[sab$d1%in%df0$drug,]
sab <- sab[sab$d2%in%df0$drug,]
