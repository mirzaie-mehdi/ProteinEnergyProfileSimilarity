...
p <- ggplot(df, aes(value, fill=variable))+
  ...
  xlab('')+ylab('Density')+
  geom_text(data = df_med, aes(x=value-.1, y=c(2.7,4.2), label = value, 
                               col=variable), size=8)+
  ...
  scale_color_manual(values = c('green4','orange'),name = "variable", labels = c("Correlation", "Cosine"))+
  scale_fill_manual(values = c('green4','orange'),name = "variable", labels = c("Correlation", "Cosine"))
