...
dis <- data.frame(pdb_mut_chain=df$pdb_mut_chain, matrix(NA,nrow(df),271))
colnames(dis)[-1] <- c('pos',
                       paste0(letters_list,'_N'),
                       paste0(letters_list,'_M'),
                       paste0(letters_list,'_NM'),pair_inter)
for (i in 1:np) {
  pdbname <- pdbs[i]
  ...
  tryCatch({
    ...
    for (k in 1:nM) {
      ...
      dis[dis$pdb_mut_chain==pmc,-1] <- c(pos/length(seqM), 
                                          ms40_N, ms40_M,ms40, d)
      }
    }, error = function(e) {
      cat("Error downloading file:", e$message, "\n")
    })
...
df <- df[!is.na(df$FF),]
# ---------------------------------
df0 <- readRDS('~/Downloads/dfs2648.rds')
colnames(df0)[5]<-'ddG'
df<-rbind(df[,-c(1:6,8:9)],df0[,-c(1:4,6:8)])
# ---------------- Reverse ----------
rev <- -1*df[,-c(1)]
colnames(rev) <- paste0(pair_inter,'_Rev')
df <- cbind(df, rev)
# ---------------------------------
...
df2 <- readRDS('~/Desktop/dfS1304.rds')
