...
```{r}
spe <- read.csv('~/Downloads/final_paywise_energy.csv',row.names = 'X0')
spe <- read.csv('Data/Dunbrack/cpe.csv')
library(Biostrings)
data(BLOSUM45)
data(BLOSUM50)
data(BLOSUM62)
...
data(PAM120)
data(PAM250)
colnames(spe)<- rownames(spe)<-aa321(colnames(spe))
x <- spe[lower.tri(spe,diag = T)]
x <- (x- min(x))/(max(x)-min(x))*(1 - (-1))+(-1)
x <- colMeans(spe[,-c(1,2)])/apply(spe[,-c(1,2)],2,sd)

sm <- PAM30[letters_list,letters_list]
y <- sm[lower.tri(sm,diag = T)]

df <- data.frame(spe=x,sc=y)
ggplot(df,aes(spe,sc))+geom_point()+xlim(c(-500,0))+stat_cor()
```
