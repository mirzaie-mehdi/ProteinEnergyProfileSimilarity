...
```{r}
df <- read.csv('~/Downloads/S9028.csv')
...
dis <- data.frame(pdb_mut_chain=df$pdb_mut_chain, dis=NA, disE=NA)
for (i in 1:length(pdbs)) {
  ...
  tryCatch({
    ...
    for (k in 1:nM) {
      ...
      # ---------------dis----------
      d <- manhat(aa210_N, aa210_M)
      pmc <- paste0(pdbname,'_',mut,'_',ch)
      dis$dis[dis$pdb_mut_chain==pmc] <- d
      dis$disE[dis$pdb_mut_chain==pmc] <- sum(aa210_N) - sum(aa210_M)
      }
    }, error = function(e) {
     cat("Error downloading file:", e$message, "\n")
    })
