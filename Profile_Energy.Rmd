...
#----------------------------------------------
num_columns <- 210
E210_spike <- data.frame(pdbID=spike$pdbID, length=NA,
                         virus=spike$virus, seq=NA,
                            matrix(NA,ncol=num_columns,nrow=Nprotein))

colnames(E210_spike)[4:214] <- pair_inter
...
for (NP in 1:Nprotein) {
  ...
  tryCatch({
    ...
    E210_spike$seq[NP] <- seq
    E210_spike$length[NP] <- nchar(seq)
    Net_Frame <- NetworkFrame(PDB = new_pdb)
    energy210 <- Energy_PC210(Net_Frame,energy_dell_dunbrack)
...
E210.seq_spike <- data.frame(E210_spike[,1:4],
                            matrix(NA,ncol=num_columns,nrow=Nprotein))
