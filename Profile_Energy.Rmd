...
# Parse Scope 2.08
## ASTRAL95_parse
This chunk parse the astral 95/40 data downloaded form Scope 2.08 and organize it into a suitable data-frame with columns "scopeID","pdbID" ,"Position", "class","fold","superfamily","family","seq". Since we already made astral_95.rds, we do not need to run the following chunk.
...
tictoc::toc()
# saveRDS(dis,'Data/rds/dis.CT_Ho.rds')
...
for (NP in 1:Nprotein) {
  ...
  tryCatch({
    ...
    #E210_ferritin$seq[NP] <- paste0(seq,collapse = '')
...
# -------- At first, run the 'Energy Calculation prerequisite' chunk.
# ----------------------------------------------------
spike <- read.csv("Data/csv/spike_close.csv")
Nprotein <- nrow(spike)
#----------------------------------------------
num_columns <- 210
E210_spike_close <- data.frame(spike,
                            matrix(NA,ncol=num_columns,nrow=Nprotein))

colnames(E210_spike_close)[6:215] <- pair_inter

tictoc::tic()
for (NP in 1:Nprotein) {
  print(NP)
  ch <- substr(E210_spike_close$pdbID[NP],5,5)
  pdbname <- substr(E210_spike_close$pdbID[NP],1,4)
  tryCatch({
    pdb0 <- read.pdb(tolower(pdbname))
    #pdb0 <- read.pdb(paste0('../covpdb/', pdbname, '.pdb'))
    sele1 <- atom.select(pdb0,"protein",type = "ATOM", chain=ch)
    sele2 <- atom.select(pdb0, "protein", elety=c("H","OXT"),inverse=TRUE)
    sele4 <- atom.select(pdb0, "noh")
    sele <- combine.select(sele1, sele2,sele4, operator="AND")
    ...
    Net_Frame <- NetworkFrame(PDB = new_pdb)
    energy210 <- Energy_PC210(Net_Frame,energy_dell_dunbrack)
    E210_spike_close[NP,6:215] <- energy210
    }, error = function(e) {
     cat("Error downloading file:", e$message, "\n")
    })
}
...
E210.seq_spike_close <- data.frame(E210_spike_close[,1:5],
                            matrix(NA,ncol=num_columns,nrow=Nprotein))
colnames(E210.seq_spike_close)[6:215] <- pair_inter

tictoc::tic()
for (NP in 1:Nprotein) {
  print(NP)
  seq <- unlist(strsplit(E210.seq_spike_close$seq[NP],''))
  freq <- c()
  for (j in 1:20){
    freq[j] <-  length(grep(tolower(letters_list[j]),tolower(seq)))
    }
  ...
  pair_es <- diag(freq) %*% as.matrix(en_fr)
  aa210_p <- pair_es[lower.tri(pair_es,diag = T)]
  E210.seq_spike_close[NP,6:215] <- aa210_p
}
df <- E210.seq_spike_close
x <- df[,-c(1:5)]/df$length
x  <- as.matrix(x) 
dis <- x %>% proxy::dist(method = manhat)
tictoc::toc()

saveRDS(E210_spike_close,file = "Data/rds/E210_spike_close.rds")
#saveRDS(E210_spike_close_SS,file = "Data/rds/E210_spike_close_SS.rds")
saveRDS(E210.seq_spike_close,file = "Data/rds/E210.seq_spike_close.rds")
write.csv(E210.seq_spike_close[,1:5], "Data/csv/spike_close.csv",row.names = F)
```
