...
dis <- as.matrix(dist(df[,c(3,11:220)]/df$length, method = manhat))
diag(dis) <- NA
...
save(ump_df_seq40,ump_df_seq95,ump_df_str40,ump_df_str95,file = 'Data/rds/UMPs.RData')
...
ump <- umap(d = dis,
            ...
            min_dist = 0.1,
            input="dist")
ump_df_str <- data.frame(data='Astral 40',type='Structure',
                           class=df[,1],
                           UMAP1 = ump$layout[, 1],
                           UMAP2 = ump$layout[, 2])
...
ump <- umap(d = dis,
            ...
            min_dist = 0.1,
            input="dist")
ump_df_seq <- data.frame(data='Astral 40',type='Sequence',
                           class=df[,1],
                           UMAP1 = ump$layout[, 1],
                           UMAP2 = ump$layout[, 2])
...
ump <- umap(d = dis,
            ...
            min_dist = 0.1,
            input="dist")
ump_df_str <- data.frame(data='Astral 95',type='Structure',
                           class=df[,1],
                           UMAP1 = ump$layout[, 1],
                           UMAP2 = ump$layout[, 2])
...
ump <- umap(d = dis,
            ...
            min_dist = 0.1,
            input="dist")
ump_df_seq <- data.frame(data='Astral 95',type='Sequence',
                           class=df[,1],
                           UMAP1 = ump$layout[, 1],
                           UMAP2 = ump$layout[, 2])
z <- rbind(ump_df_str,ump_df_seq)
...
legend("topright", legend = as.character(unique(ord_sml)),
       pch = 15, pt.cex = 2.3, cex = 1.4, bty = 'n',
       inset = c(-.3, -.1), y.intersp=0.1, #c(.3, -.1)
       col = unique(group_cols),horiz = FALSE,xpd = TRUE)
```
...
p3<-ggboxplot(df2, x='class',y='total', 
  ...
  theme(legend.position = 'none')+ylim(-6500,2000)+
  scale_fill_viridis(discrete = TRUE, alpha=0.6)+
  stat_compare_means(comparisons = my_comparisons, method = 't.test',
                     #label.y = c(-6000,-8000,-10000),
