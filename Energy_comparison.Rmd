...
dis <- as.matrix(dist(df[,-c(1:2,4:8)],method = manhat_norm))
...
#  dis[i] <- d
  ...
  d <- dist(df,method = manhat_norm)
  dis_new[i] <- d
  d <- dist(df,method = manhat_norm_min)
  dis_new_min[i] <- d
...
df <- data.frame(group=rep(c('rand40','mulidup'),c(241,241)),
...
```{r}
calculate_tm_score <- function(pdb_file1, pdb_file2) {
  # Define the path to the TMalign executable
  tmalign_path <- "/path/to/TMalign"  # Replace with the actual path

  # Run TMalign externally
  tmalign_cmd <- sprintf("%s %s %s", tmalign_path, pdb_file1, pdb_file2)
  tmalign_output <- system2(command = tmalign_cmd, stdout = TRUE)

  # Extract TM-score from the output
  tm_score <- as.numeric(strsplit(tmalign_output[grepl("TM-score", tmalign_output)], ":")[[1]][2])

  return(tm_score)
}

# Example usage
pdb_file1 <- "path/to/your/protein1.pdb"
pdb_file2 <- "path/to/your/protein2.pdb"

tm_score <- calculate_tm_score(pdb_file1, pdb_file2)

# Print the TM-score
cat("TM-Score:", tm_score, "\n")
