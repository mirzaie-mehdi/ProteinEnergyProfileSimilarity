...
# ************************************
# *********************************
my_tuneRF<- function (x, y, mtryStart = if (is.factor(y)) floor(sqrt(ncol(x))) else floor(ncol(x)/3), 
    ...
    plot = TRUE, doBest = FALSE, ...) 
{
    ...
    for (direction in c("left", "right")) {
        ...
        while (Improve >= improve) {#
            ...
            if (mtryCur == mtryOld | errorOld<0.01) 
                break
            errorCur <- if (classRF) {
...
```{r}
df <- readRDS("data/rds/E210.astral40.rds")
df = df[!is.na(df$FL) & df$class %in% c('a','b','c','d',"e","f","g"), ]
...
for (i in 1:length(folds)) {
  print(i)
  x <- df[df$fold %in% folds[i], ]
  dis <- proxy::dist(x[ ,-c(1:8)],method = tm_score3)
  within <- c(within,as.vector(dis))
}

dis <- as.matrix(proxy::dist(df[ ,-c(1:8)],method = tm_score3))
dis[lower.tri(dis,diag = T)] <- NA
dis <- data.frame(fold=df$fold,dis)
between<-c()
for (i in 1:length(folds)) {
  print(i)
  fo <- folds[i]
  cl <- substr(fo,1,1)
  #idxcl <- which(df$class==cl)
  idx <- which(dis$fold==fo)
  x <- dis[idx,-c(1,idx+1)]
...
df_fold <- data.frame(gr=rep(c('within','between'),c(length(wit),length(bet))),
                    distance=c(wit,bet))
df_fold$score <- 1/(1+(df_fold$distance))

ggplot(df_fold,aes(gr,score,fill=gr))+geom_boxplot(alpha=.3)
