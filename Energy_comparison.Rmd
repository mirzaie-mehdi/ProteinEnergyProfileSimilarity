...
=======
# ***********
>>>>>>> 8813a7f779e05eb34ba8290c1e32f401acab9f89
manhat <- function(X, Y){
  #x = X[-1];y = Y[-1]
  x = X; y = Y
  xy = abs(x - y)
...
x = df[,-c(1,3:4)]
x  <- as.matrix(x) 
hc <- x %>% proxy::dist(method = manhat_norm_maxl)
hc[is.nan(hc)] <- 0
...
dis <- as.matrix(proxy::dist(df1, method = manhat_norm_maxl))
ump <- umap(d = dis,
            n_neighbors = 40,
            min_dist = 0.01,
            input="dist")
...
dis <- as.matrix(dist(df[,-c(1:8)],method = manhat))
...
df_diff_same = rbind(pairs_random_differ,pairs_random_same)
ggplot(data = df_diff_same,aes(LengthDifference,Distance,color = group)) +
  geom_point()

diff_fold <- different_proteins[sample(nrow(different_proteins), size = 4000, replace = FALSE), ]
...
```{r}
mulidup <- read.delim('Data/dup/dup.txt')
mulidup_tm <- read.csv("Data/dup/dup_tm.csv", header=T)

E210_mulidup <- data.frame(pair.name='x',pdbID=1:482,scopeID='x',position='x',length=0,
                   class='x',fold='x',superfamily='x',family='x',
                   matrix(0,482,210))
colnames(E210_mulidup)[-c(1:9)] <- pair_inter
E210.seqPDB_mulidup <- E210_mulidup

for(i in 1:nrow(mulidup)){
  print(i)
  r1 <- 2*i-1
  r2 <- 2*i
  path <- paste0('Data/dup/', mulidup$Pair.Name[i])
  ...
  pdb2 <- trim.pdb(pdb2, sele,verbose = FALSE)
  # ******
  E210_mulidup$pdbID[r1] <- mulidup$Domain1[i]
  E210_mulidup$pdbID[r2] <- mulidup$Domain2[i]
  
  E210_mulidup$scopeID[r1] <- mulidup$SCOP.ID1[i]
  E210_mulidup$scopeID[r2] <- mulidup$SCOP.ID2[i]
  
  E210_mulidup$position[r1] <- mulidup$Range1[i]
  E210_mulidup$position[r2] <- mulidup$Range2[i]
  
  E210_mulidup$length[r1] <- length(unique(pdb1$atom$resno))
  E210_mulidup$length[r2] <- length(unique(pdb2$atom$resno))
  
  E210_mulidup$pair.name[r1:r2] <- mulidup$Pair.Name[i]
  E210_mulidup$class[r1:r2] <- mulidup$SCOP.Class[i]
  E210_mulidup$fold[r1:r2] <- mulidup$SCOP.Fold[i]
  E210_mulidup$superfamily[r1:r2] <- mulidup$SCOP.Superfamily[i]
  E210_mulidup$family[r1:r2] <- mulidup$SCOP.Family[i]
...
#  }
  energy210 <- Energy_PC210(Net_Frame,energy_dell_dunbrack)
  E210_mulidup[r1, -c(1:9)] <- energy210
...
#  }
  energy210 <- Energy_PC210(Net_Frame ,energy_dell_dunbrack)
  E210_mulidup[r2, -c(1:9)] <- energy210
  # ******************************
  #-----------sequence enregy based on PDB--------
  pdbX <- pdb1$atom
  pdbXca <- pdbX[pdbX$elety == "CA",]
  ...
  pair_es <- diag(freq) %*% as.matrix(en_fr)
  aa210_p<-pair_es[lower.tri(pair_es,diag = T)]
  E210.seqPDB_mulidup[r1,-c(1:9)] <- aa210_p
  ...
  pair_es <- diag(freq) %*% as.matrix(en_fr)
  aa210_p<-pair_es[lower.tri(pair_es,diag = T)]
  E210.seqPDB_mulidup[r2,-c(1:9)] <- aa210_p
}

E210.seqPDB_mulidup[,c(1:9)] <- E210_mulidup[,c(1:9)]
saveRDS(E210_mulidup,'Data/rds/E210_malidup.rds')
saveRDS(E210.seqPDB_mulidup,'Data/rds/E210.seqPDB_malidup.rds')
...
dis <- as.matrix(dist(df[,-c(1:4,6:9)], method = manhat_norm_minl))
diag(dis)<-NA
dis <- data.frame(gr=df$superfamily,dis)
gr <- unique(dis$gr)
...
ggplot(df_family,aes(gr,distance))+geom_boxplot()
# **********


im <- apply(dis,1, FUN = which.min)
res <- data.frame(sorce=df$fold, target=df$fold[im])
res$pred <- ifelse(res[,1]==res[,2],"1","0")
sum(as.numeric(res$pred))
=======
plot(dis_old, mulidup_tm$tm_norm_chain1)
>>>>>>> 8813a7f779e05eb34ba8290c1e32f401acab9f89
...
df <- data.frame(group=rep(c('rand40','mulidup','tm'),c(241,241,241)),
                 distance=c(dis40[sample(1000,241)], dis_old,
                            mulidup_tm$tm_norm_chain1))
...
df_family_str <- data.frame(data='Astral 40-family a.29.3',type='Structure',
...
df_family_seq <- data.frame(data='Astral 40-family a.29.3',type='Sequence',
                            gr=rep(c('within','between'),
                                   c(length(within),length(between))),
                            distance=c(within,between)) 
# **********
...
df_family_str <- data.frame(data='Astral 95-family a.29.3',type='Structure',
                            gr=rep(c('within','between'),
                                   c(length(within),length(between))),
                            distance=c(within,between)) 
# ****** sequence95
...
df_family_seq <- data.frame(data='Astral 95-family a.29.3',type='Sequence',
                            gr=rep(c('within','between'),
                                   c(length(within),length(between))),
                            distance=c(within,between)) 
# **********
...
df_superfamily_str <- data.frame(data='Astral 40-superfamily a.29',type='Structure',
                            gr=rep(c('within','between'),
                                   c(length(within),length(between))),
                            distance=c(within,between)) 
# ****** sequence40
...
df_superfamily_seq <- data.frame(data='Astral 40-superfamily a.29',type='Sequence',
                            gr=rep(c('within','between'),
                                   c(length(within),length(between))),
                            distance=c(within,between)) 
# **********
...
df_superfamily_str <- data.frame(data='Astral 95-superfamily a.29',type='Structure',
                            gr=rep(c('within','between'),
                                   c(length(within),length(between))),
                            distance=c(within,between)) 
# ****** sequence95
...
df_superfamily_seq <- data.frame(data='Astral 95-superfamily a.29',type='Sequence',
                            gr=rep(c('within','between'),
                                   c(length(within),length(between))),
                            distance=c(within,between)) 
# **********
...
df_fold_str <- data.frame(data='Astral 40-class alpha',type='Structure',
...
wit <- within[sample(length(within),1000)] 
bet <- between[sample(length(between),1000)] 
df_fold_seq <- data.frame(data='Astral 40-class alpha',type='Sequence',
                            gr=rep(c('within','between'),c(1000,1000)),
                            distance=c(wit,bet))
# **********
...
wit <- within[sample(length(within),1000)] 
bet <- between[sample(length(between),1000)] 
df_fold_str <- data.frame(data='Astral 95-class alpha',type='Structure',
                            gr=rep(c('within','between'),c(1000,1000)),
                            distance=c(wit,bet))
# ****** sequence95
df <- readRDS("Data/rds/E210.seq_astral_95.rds") 
...
wit <- within[sample(length(within),1000)] 
bet <- between[sample(length(between),1000)] 
df_fold_seq <- data.frame(data='Astral 95-class alpha',type='Sequence',
                            gr=rep(c('within','between'),c(1000,1000)),
                            distance=c(wit,bet)) 
# **********
df_fold <- rbind(df_fold_seq, df_fold_str)
...
# ******** str40
df <- readRDS("Data/rds/E210.astral40.rds")
df <- df[df$class %in% c("a",'b')&!is.na(df$FF)&df$length>10,]
dis <- as.matrix(proxy::dist(df[,sig_int], method = manhat))
...
homo <- data.frame(type='homo',distance=within)
remote_homo <- data.frame(type='remote_homo',distance=between)
# *************
dis$gr <- df$fold
gr <- unique(dis$gr)
between<-c() 
...
str_analog <- data.frame(type='str_analog',distance=between)
str_analog_fold <- data.frame(type='str_analog_fold',distance=between)

between_class <- data.frame(type='between_class',distance=between)
within_class <- data.frame(type='within_class',distance=within)

df_evolution <- rbind(homo[sample(nrow(homo),5000),],
                      remote_homo[sample(nrow(remote_homo),5000),],
                      str_analog[sample(nrow(str_analog),5000),],
                      str_analog_fold[sample(nrow(str_analog_fold),5000),],
                      between_class[sample(nrow(between_class),5000),],
                      within_class[sample(nrow(within_class),5000),])

my_comparisons <- list(c('between_class','within_class'))
g.p<-ggboxplot(df_evolution, x='type',y='distance', 
          fill = 'type',alpha=.5,outlier.shape = NA)+
  stat_summary(fun.y=mean, geom="point", shape=8,
               size=2, color="black", fill="red")+xlab('')+
