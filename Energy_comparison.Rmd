...
k=3
predicted_classes <- knn(train = train_data[, c("x1", "x2")], test = test_data[, c("x1", "x2")], cl = train_data$class, k = k)
...
df <- df[,-c(1:4,6:8),]
validationIndex <- createDataPartition(df$class, p=0.70, list=FALSE)
train <- df[validationIndex,] # 70% of data to training
test <- df[-validationIndex,] # remaining 30% for test
# Run algorithms using 10-fold cross validation
trainControl <- trainControl(method="repeatedcv", number=10, repeats=3)
metric <- "Accuracy"
set.seed(7)
...
k <- 3
# Set the distance metric (e.g., "manhattan" or "maximum")
distance_metric <- "manhattan"
# Run KNN classifier with the specified distance metric
predicted_classes <- knn(train = train_data[, c("x1", "x2")], 
                         test = test_data[, c("x1", "x2")], 
                         cl = train_data$class, 
                         k = k)
...
k_grid <- expand.grid(k = seq(from = 1, to = 10, by = 1))
manhat <- function(x, y){
#  x = X[-1];y = Y[-1]
  xy = abs(x - y)
  xy = xy[xy != 0]
...
ManDKNN <- function(train_data, test_data, k) {
  predictions <- vector("character", nrow(test_data))
  for (i in 1:nrow(test_data)) {
    new_input <- test_data[i, -1]
    distances <- apply(train_data[,-1], 1, 
                       function(x) manhat(x, new_input))
    DL_table <- data.frame(dist = distances, label = train_data$class)
  ...
  for (i in 1:nrow(test)) {
    new_input <- test[i, -2]
    df1  = rbind(new_input,train[,-2])
    dis <- as.matrix(dist(df1, method = manhat))

    distances <- apply(train[,-1], 1, 
                       function(x) manhat(x, new_input))
    DL_table <- data.frame(dist = distances, label = train$class)
    sorted_DL_table <- DL_table[order(DL_table$dist), ]
    neighbors <- head(sorted_DL_table, k)
    predictions[i] <- names(which.max(table(neighbors$label)))
    predictions[i]
...
ED <- ManDKNN(train_data = train, 
            test_data = test, k)

tableManD <- table(ED, test$class)
...
row.names(df) <- df[,1]
rn <- df[,1]
dis <- proxy::dist(df[,-c(1,3)], method = manhat_norm_diff)
dis[is.nan(dis)]<-0
tree_data <- ape::bionj(dis)
