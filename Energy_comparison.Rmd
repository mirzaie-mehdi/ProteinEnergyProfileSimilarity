...
dfs <- df[df$class %in% c("a","b")& df$length > 80 & !is.na(df$FF),]
df1<-dfs[,-c(1:5,7:8)]
...
dis <- as.matrix(proxy::dist(df1[,-c(1,2)], method = tm_score3))
#dis = 1 - dis/ max(dis)
ump <- umap(d = dis,
            n_neighbors = 30,
            min_dist = 0.1,
...
dis <- proxy::dist(df[,-c(1,2,3)], method = tm_score3)
dis[is.nan(dis)]<-0
tree_data <- ape::bionj(dis)
...
x  <- as.matrix(x) 
hc <- x %>% proxy::dist(method = tm_score3)
hc[is.nan(hc)] <- 0
...
dis <- as.matrix(proxy::dist(df1, method = tm_score3))
ump <- umap(d = dis,
            n_neighbors = 40,
            min_dist = 0.01,
            input="dist")
...
df = E210.seq_bacteriocin
x = df[,-c(1:2)]
hc <- as.matrix(x %>% dist(method = 'manhattan'))
...
# Mulidup
```{r}
mulidup <- read.delim('~/Desktop/rds/dup/dup.txt')
