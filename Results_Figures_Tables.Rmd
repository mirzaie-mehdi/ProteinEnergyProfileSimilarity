...
df_tot <- readRDS('manuscript/Revised/fig_data/df_total_energy.rds')
...
AB <- ggplot(df_tot, aes(seq/2000,str/1000))+
  ...
  mytheme()
# ************** distance 40 and 95
df_dis40<-readRDS('manuscript/Revised/fig_data/dis_40_manhat.rds')
df_dis95<-readRDS('manuscript/Revised/fig_data/dis_95_manhat.rds')
...
source('Functions.R')
# ----------------------------------------------------
load('manuscript/Revised/fig_data/UMPs.RData')
...
A<-ggarrange(NULL,NULL,NULL,NULL,
  ...
  font.label = list(size = 30),
  hjust = .2,vjust = 0)
#saveRDS(A,'manuscript/fig_data/Fig4A.rds')
saveRDS(A,'manuscript/Revised/fig_data/Fig4A.rds')
```
...
B <- ggarrange(NULL,NULL,NULL,NULL,
  ...
  font.label = list(size = 30),
  hjust = .2,vjust = 0)
saveRDS(B,'manuscript/Revised/fig_data/Fig4B.rds')
```
...
C <- ggarrange(NULL,NULL,NULL,NULL,
  ...
  font.label = list(size = 30),
  hjust = .2,vjust = 0)
saveRDS(C,'manuscript/Revised/fig_data/Fig4C.rds')
```
...
```{r Fig 4 ABC}
rm(list = ls())
A <- readRDS('manuscript/Revised/fig_data/Fig4A.rds')
B <- readRDS('manuscript/Revised/fig_data/Fig4B.rds')
C <- readRDS('manuscript/Revised/fig_data/Fig4C.rds')
# ------------------------
...
saveRDS(mylegend,'manuscript/Revised/fig_data/Fig5mylegend.rds')
# ******************************************
# ****** structure95
df <- readRDS("Data/rds/E210.astral_95.rds") 
df = df[!is.na(df$FL),] 
...
saveRDS(EF,'manuscript/Revised/fig_data/Fig5EF.rds')
```
...
saveRDS(CD,'manuscript/Revised/fig_data/Fig5CD.rds')
```
...
saveRDS(AB,'manuscript/Revised/fig_data/Fig5AB.rds')
```
...
```{r Fig 5 All}
rm(list = ls())
EF <- readRDS('manuscript/Revised/fig_data/Fig5EF.rds')
CD <- readRDS('manuscript/Revised/fig_data/Fig5CD.rds')
AB <- readRDS('manuscript/Revised/fig_data/Fig5AB.rds')
mylegend <-readRDS('manuscript/fig_data/Fig5mylegend.rds')
# ---------------------------------------------
f5all <- ggarrange(NULL,AB,CD,EF,ncol = 1,
          heights = c(0.1,1,1,1))
p<-cowplot::plot_grid(f5all, mylegend, ncol = 1, rel_heights = c(1,.03))
# save pdf 10x9
```
...
adjustedRandIndex(cutree(hc, 5), df$virus)
#classError(cutree(hc, 4), df$virus)
...
#               hjust=1,size=1.5,fontface="bold",align=F)+
  ...
  geom_text2(aes(subset=!isTip, label=ci),
             col='black', hjust=-.3, vjust=-.5)+
  scale_color_discrete(breaks=c('MERS_Cov', 'SARS_Cov', 'SARS_Cov2'),
                      type =  c('gray45',"blue","green","red" ))+
  ggtitle('CPE')
# -----------------------------------------------------
# ------------ SPE ----------------
...
#   myBoots <- boot.phylo(tree_data, as.matrix(rms),
#                       FUN = function(d){
#                         as.phylo(hclust(rms,method = 'ward.D2'))},
#                       B = 100)
#   p_data$data$boots <- c(rep(NA,143),myBoots)
...
t_tmVec <- groupOTU(p_data, grp, "groups") + aes(color=groups) +
       theme(legend.position="none",
             plot.title = element_text(size = 50, face = 'bold',hjust = 0.5,vjust = 2))+
  geom_text2(aes(subset=!isTip, label=boots),
             col='black', hjust=-.2, vjust=-.5, size=8)+
  geom_text2(aes(subset=!isTip, label=ci),
             col='black', hjust=-.2, vjust=.5)+
       scale_color_discrete(breaks=c('MERS_Cov', 'SARS_Cov', 'SARS_Cov2'),
                            type =  c("blue","green","red" ))+
...
#   title_grob <- grobTree(
#     textGrob("Evolution of Spike protein", x = 0.5, y = -3, just = "center", 
#              gp = gpar(col = "black", fontsize = 45, fontface = "bold")),
#     textGrob("MERS_Cov",           x = 0.34, y = -9,   just = "center", 
#              gp = gpar(col = "blue",  fontsize = 30, fontface = "bold")),
#     textGrob(expression('\u2192'), x = 0.42, y = -9.5, just = "center", 
#              gp = gpar(col = "black", fontsize = 40, fontface = "bold")),
#     textGrob("SARS_Cov",           x = 0.50, y = -9,   just = "center", 
#              gp = gpar(col = "green", fontsize = 30, fontface = "bold")),
#     textGrob(expression('\u2192'), x = 0.58, y = -9.5, just = "center", 
#              gp = gpar(col = "black", fontsize = 40, fontface = "bold")),
#     textGrob("SARS_Cov2",          x = 0.67, y = -9,   just = "center", 
#              gp = gpar(col = "red"  , fontsize = 30, fontface = "bold"))
#   )
...
p190s <- p12h+xlim(c('CPE','TM-Vec','SPE'))+
  ylim(0,.44)+ #4.4*12*3.6 = 190sec
    theme(legend.text = element_text(face = 'bold',size = 15))
...
# ---------------------------------------
set.seed(12345)
df5[,-c(1:9)] <- df5[,-c(1:9)]/df5$length
