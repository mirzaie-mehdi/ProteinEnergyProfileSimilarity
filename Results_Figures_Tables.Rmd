...
p1 <- ggplot(res, aes(length, disTot))+
  ...
  theme_bw(base_line_size = .3)+
  mytheme()+
  ylab('Distance Totoal Energy')+
  #xlim(0,1000)+
  ylim(-.5,.5)
# -------------------------
length_var <- res$length
other_vars <- res[, 54:263]
...
EF <- ggarrange(NULL,NULL,NULL,p1,NULL,p2, ncol = 3,nrow = 2,
                heights = c(.05,1),widths = c(1,.05,.7),
                labels = c('','','','C','','D'),
                font.label = list(size = 30),
                hjust = -.7,vjust = 0)
# --------------------------
p<-ggarrange(NULL,
             AB, NULL,CD, NULL, EF, ncol = 1,
             heights = c(0.09, 1, 0.17, 1, .17, 1),
             labels = c('',"A", "", "B", '',''),
...
adjustedRandIndex(cutree(upgma, 3), df$virus)
